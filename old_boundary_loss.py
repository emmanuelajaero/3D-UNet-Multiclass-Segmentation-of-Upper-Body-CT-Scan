...
def compute_sdf(img_gt, out_shape):
    ...
    for p in range(out_shape[0]):  # patch size
        for b in range(out_shape[1]):  # batch
            for c in range(out_shape[2]):   # channel
                posmask = img_gt[p][b][c].astype(np.bool)
                if posmask.any():
                    negmask = ~posmask
...
class BDLoss(nn.Module):
    ...
    def forward(self, net_output, gt):
        ...
        multipled = torch.einsum("pbcxyz,pbcxyz->pbcxyz", net_output[:, :, ...], phi[:, :, ...])
