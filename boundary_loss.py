...
def get_tp_fp_fn(net_output, gt, axes=None, mask=None, square=False):
    ...
    with torch.no_grad():
        if len(shp_x) != len(shp_y):
            gt = gt.view((shp_y[0], shp_y[1], *shp_y[2:]))
...
def distance_transform(mask):
    # Compute pairwise distances between all points in mask
    mask = mask.float()
    dist_matrix = torch.cdist(mask.nonzero(), mask.nonzero(), p=2)

    # Reshape distance matrix to shape of input mask
    dist = torch.zeros_like(mask)
    for i, j in zip(*mask.nonzero(as_tuple=True)):
        dist[i, j] = dist_matrix[(mask[i, j] - 1), (mask[i:, j:].sum(dim=1).nonzero(as_tuple=True)[0] + i)].min()
...
def compute_sdf(img_gt, out_shape):
    ...
    for p in range(out_shape[0]):  # patch size
        for c in range(1, out_shape[1]):  # channel
            posmask = img_gt[p][c].astype(np.bool)
            if posmask.any():
                negmask = ~posmask
                posdis = distance_transform_edt(posmask)
                negdis = distance_transform_edt(negmask)

                # posdis = distance_transform(torch.tensor(posmask))
                # negdis = distance_transform(torch.tensor(negmask))
...
class BDLoss(nn.Module):
    ...
    def forward(self, net_output, gt):
        ...
        multipled = torch.einsum("pcxyz,pcxyz->pcxyz", net_output[:, 1:, ...], phi[:, 1:, ...])
        min_val = np.nanmin(multipled)
        max_val = np.nanmax(multipled)
        normalized_multipled = (multipled - min_val) / (max_val - min_val)
        bd_loss = normalized_multipled.nanmean()
        return bd_loss
